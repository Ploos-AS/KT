...
void kt_term_present_scheduler_reset(kt_term_present_scheduler*s){if(s){s->pending=0;s->immediate=0;s->blink_pending=0;s->first_row=0;s->count=0;s->policy=KT_TERM_PRESENT_IMMEDIATE;}}
void kt_term_present_scheduler_set_policy(kt_term_present_scheduler*s,kt_term_present_policy p){if(s&& (p==KT_TERM_PRESENT_IMMEDIATE||p==KT_TERM_PRESENT_COALESCE)){s->policy=p;s->immediate=(uint8_t)(p==KT_TERM_PRESENT_IMMEDIATE);}}
int kt_term_present_scheduler_add_rows(kt_term_present_scheduler*s,const kt_term_dirty_rows*d){
 ...
 end=(uint32_t)d->first+d->count;if(!s->pending){s->first_row=d->first;s->count=d->count;s->pending=1;s->immediate=(uint8_t)(s->policy==KT_TERM_PRESENT_IMMEDIATE);return 0;}
