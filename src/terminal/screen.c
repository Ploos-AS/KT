...
void kt_term_screen_clear(kt_term_screen *s){size_t i;if(!s)return;for(i=0;i<s->cell_count;i++){s->cells[i].ch=' ';s->cells[i].charset=KT_TERM_PETSCII_UPPER_GRAPHICS;s->cells[i].attr.fg=7u;s->cells[i].attr.bg=0u;s->cells[i].attr.flags=0u;}s->x=0;s->y=0;s->charset=KT_TERM_PETSCII_UPPER_GRAPHICS;}
...
static void put(void *p,uint8_t ch,const kt_term_attr *a){kt_term_screen*s=p;if(s->x>=s->width||s->y>=s->height)return;at(s,s->x,s->y)->ch=ch;at(s,s->x,s->y)->charset=s->charset;at(s,s->x,s->y)->attr=*a;if(++s->x>=s->width)advance_line(s);}
...
static void blank(kt_term_cell*c){c->ch=' ';c->charset=KT_TERM_PETSCII_UPPER_GRAPHICS;c->attr.fg=7u;c->attr.bg=0u;c->attr.flags=0u;}
...
static void charset(void*p,uint8_t cs){((kt_term_screen*)p)->charset=cs;}
static const kt_term_ops ops={put,cr,lf,bs,bell,move,pos,erase,charset};
...
uint32_t kt_term_decode_petscii(uint8_t ch,uint8_t charset){
 ...
 if(charset==KT_TERM_PETSCII_UPPER_GRAPHICS){
  static const uint32_t graphics_a0_bf[32]={
   0x00A0u,0x258Cu,0x2584u,0x2594u,0x2581u,0x258Fu,0x2592u,0x2595u,
   0x1FB8Fu,0x25E4u,0x1FB87u,0x251Cu,0x2597u,0x2514u,0x2510u,0x2582u,
   0x250Cu,0x2534u,0x252Cu,0x2524u,0x258Eu,0x258Du,0x1FB88u,0x1FB82u,
   0x1FB83u,0x2583u,0x1FB7Fu,0x2596u,0x259Du,0x2518u,0x2598u,0x259Au
  };
