...
void kt_term_geometry_clear_remote(kt_term_geometry*g){if(g){g->remote_valid=0;if(g->policy==KT_TERM_GEOMETRY_REMOTE){g->cols=g->fixed_cols;g->rows=g->fixed_rows;}}}
